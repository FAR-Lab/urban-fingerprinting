...
class AnimatedChloropleth:
    def __init__(self, graph, days=None):
        ...
        gdf = pd.concat(
            [
                doc.frames_data
                for doc in graph.days_of_coverage
                if doc.date in days
            ]
        )
        ...
        gdf = gdf.merge(
            detections, left_on="frame_id", right_index=True, how="left"
        )
    ...
    def plot_frame(self, args):
        ...
        norm = matplotlib.colors.Normalize(
            vmin=0, vmax=self.map_data[coloring_column].max()
        )
        ...
        cb.set_label(
            label=f"# of {self.curr_entity_name}s",
            size=40,
            weight="bold",
            labelpad=20,
        )
        ...
        plt.savefig(
            f"{self.frames_dir}/{frame[time_column].max().strftime('%Y-%m-%d-%H-%M-%S')}.png",
            bbox_inches="tight",
            pad_inches=0,
        )
        fig.clf()
    ...
    def generate_frames(
        ...
        try:
            ...
            rc(
                "text.latex",
                preamble=r"\usepackage{amsmath} \usepackage[T1]{fontenc}",
            )
            ...
            log.warning(
                f"LaTeX font rendering not configured correctly, error: {e}."
            )
        ...
        if car_offset:
            log.info(
                "Car offset mode enabled (counting each dashcam image as proof of one car.)"
            )
        ...
        for i in tqdm(range(len(intervals) - 1)):
            frames.append(
                self.map_data[
                    (self.map_data[time_column] >= intervals[i])
                    & (self.map_data[time_column] < intervals[i + 1])
                ]
            )
        ...
        frames = Parallel(n_jobs=8, timeout=10000)(
            delayed(self.plot_frame)(arg) for arg in tqdm(args)
        )
        return frames
