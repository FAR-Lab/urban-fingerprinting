...
class SampleOverTime(object):
    ...
    def __init__(self, sample_csv_path, md_tld): 
        ...
        self.md = pd.concat([pd.read_csv(md_path, engine='pyarrow') for md_path in self.md_paths])
        ...
        self.sample = gpd.GeoDataFrame(self.sample, geometry=gpd.points_from_xy(self.sample[LONGITUDE_COL], self.sample[LATITUDE_COL]), crs=COORD_CRS)
    ...
    def generate_frame(self, idx, df): 
        ...
        plt.savefig(f"../../output/sample_over_time/{self.id}/{idx}.png", bbox_inches='tight', pad_inches=0)
    ...
    def generate_frames(self, num_points_per_frame=1000):
        ...
        frames = np.array_split(self.sample, len(self.sample) // num_points_per_frame)

        self.log.info(f"Split sample into {len(frames)} frames of {num_points_per_frame} points each.")

        # create output directory
        self.id = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        os.mkdir(f"../../output/sample_over_time/{self.id}")


        self.log.info(f"Generating frames for {self.id}.")
        # generate frames in parallel
        Parallel(n_jobs=16)(delayed(self.generate_frame)(idx, df) for idx, df in tqdm(enumerate(frames), desc="Generating frames.", total=len(frames)))
    ...
    def generate_gif(self): 
            ...
            frames = glob.glob(f"../../output/sample_over_time/{self.id}/*.png")
            frames = sorted(frames, key=lambda x: int(x.split("/")[-1].split(".")[0]))
    
            frames = [Image.open(frame) for frame in frames]
    
            frames[0].save(f"../../output/sample_over_time/{self.id}.gif", format='GIF', append_images=frames[1:], save_all=True, duration=50, loop=0)
