...
class CoverageCard:
    def __init__(self, days_of_coverage, attached_plot=""):
        ...
        # CASE: list of datetime objects
            ...
            map(lambda x: isinstance(x, datetime.datetime), days_of_coverage)
        ):
            self.days_of_coverage = days_of_coverage
        # CASE: list of DayOfCoverage objects
        elif all(
            map(lambda x: isinstance(x, DayOfCoverage), days_of_coverage)
        ):
            self.days_of_coverage = list(
                map(lambda x: x.date, days_of_coverage)
            )
            ...
            raise ValueError(
                "days_of_coverage must be a list of strings, datetimes, or DayOfCoverage objects"
            )
    ...
    def plot_within_week(self):
        ...
        ax.set_xticklabels(
            [
                (earliest_day + datetime.timedelta(days=x)).strftime(
                    "%a %m-%d-%y"
                )
                for x in range(7)
            ]
        ...
        # plot coverage
        for day in [
            earliest_day + datetime.timedelta(days=x) for x in range(7)
        ]:
            if day in self.days_of_coverage:
                ax.axvspan(
                    day.weekday() + padding_h,
                    day.weekday() + (1 - padding_h),
                    ymin=padding_v,
                    ymax=box_height - padding_v,
                    facecolor="green",
                    alpha=0.5,
                    capstyle="round",
                )
    ...
    def plot_within_month(self, data=None, ax=None, save=True):
        ...
        num_days_in_month = calendar.monthrange(
            earliest_day.year, earliest_day.month
        )[1]
        for day in [
            first_of_month + datetime.timedelta(days=x)
            for x in range(num_days_in_month)
        ]:
            ...
            if day in data:
                ax.add_patch(
                    mpatches.Rectangle(
                        (
                            col * box_width + outer_padding_h,
                            row * box_height + outer_padding_v,
                        ),
                        box_width,
                        box_height,
                        facecolor=self.cosmetics["covered_color"],
                        alpha=self.cosmetics["covered_alpha"],
                        capstyle="round",
                    )
                )
                ...
                # add label inplace
                ax.text(
                    col * box_width + outer_padding_h + box_width / 2,
                ...
                ax.add_patch(
                    mpatches.Rectangle(
                        ...
                        box_width,
                        box_height,
                        facecolor=self.cosmetics["missing_color"],
                        alpha=self.cosmetics["missing_alpha"],
                        capstyle="round",
                    )
                )
                ax.add_patch(
        ...
        green_patch = mpatches.Patch(
            color=self.cosmetics["covered_color"], label="Day is in Plot"
        )
        red_patch = mpatches.Patch(
            color=self.cosmetics["missing_color"], label="Day Excluded"
        )
        plt.legend(
        ...
        for i in range(7):
            i_day = (i - 1) % 7
            ax.text(
                i * box_width + outer_padding_h + box_width / 2,
                5.65 * box_height + outer_padding_v + box_height / 2,
                calendar.day_abbr[i_day],
                horizontalalignment="center",
                verticalalignment="center",
            )
        ...
        if save:
            plt.savefig(
                f"{Path(self.attached_plot).stem}_coverage_card.png",
                bbox_inches="tight",
            )
