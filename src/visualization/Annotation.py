...
class AnnotationViewer: 
    ...
    def get_annotations(self, frame_id, yolo_detections_path=""):
        ...
        try: 
            frame_id = glob(f"/share/ju/urbanECG/output/yolo/2023-08-18/*/exp/labels/{frame_id}.txt")[0]
        except IndexError:
    ...
    def visualize_annotations(self, frame_id, annotations, class_id=-1):
        ...
        for _, annotation in annotations.iterrows(): 
            ...
            # plot bounding box
            rect = plt.Rectangle(bottom_left, w, h, fill=False, edgecolor=color, linewidth=2)
            ax.add_patch(rect)


            # plot rectangle behind text 
            text_bbox = ax.text(top_center[0], top_center[1], f"{class_id} [{conf}]", color='black')
            text_bbox.set_bbox(dict(facecolor=color, alpha=0.5, edgecolor=color))

            # antialiasing
            text_bbox.set_path_effects([path_effects.Stroke(linewidth=2, foreground='white'),
                                        path_effects.Normal()])
            ...
            plt.savefig(f"../../output/annotation_viewer/{frame_id}.png", bbox_inches='tight', pad_inches=0)
