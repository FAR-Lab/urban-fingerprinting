...
def coverage_growth_over_time(G):
    ...
    for DoC in G.days_of_coverage: 
        if len(roads_covered) == 0:
            roads_covered = DoC.nearest_edges.groupby(["u", "v"]).size().reset_index(name=f"count_{DoC.date}")
        else: 
            roads_covered = roads_covered.merge(DoC.nearest_edges.groupby(["u", "v"]).size().reset_index(name=f"count_{DoC.date}"), on=["u", "v"], how="outer")
        
        G.log.info(f"After adding {DoC.date}, {len(roads_covered.index)} roads covered")
    ...
    G.log.info(f"Saving coverage growth to output/coverage_growth/{len(G.days_of_coverage)}_days_of_coverage_{timestamp}.csv...")
    df.to_csv(f'../../output/coverage_growth/{len(G.days_of_coverage)}_days_of_coverage_{timestamp}.csv')
...
def plot_coverage_growth_over_time(data_frame, graph):
    ...
    ax.plot(data_frame.index, data_frame.roads_covered, color='b', label="Roads Covered")

    # Add dashed horizontal line at 100% coverage.
    ax.axhline(y=len(graph.gdf_edges), color='r', linestyle='--', label="100% Coverage")
    ...
    plt.legend()
    plt.savefig(f"../../output/coverage_growth/figures/{len(graph.days_of_coverage)}_days_of_coverage_{timestamp}.png")
...
if __name__ == "__main__":
    ...
    augDoCs = ["2023-08-10", "2023-08-11", "2023-08-12", "2023-08-13", "2023-08-14", "2023-08-17", "2023-08-18", "2023-08-20", "2023-08-21", "2023-08-22", "2023-08-23", "2023-08-24", "2023-08-28", "2023-08-29", "2023-08-30", "2023-08-31"]
