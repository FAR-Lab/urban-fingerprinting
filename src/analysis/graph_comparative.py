...
class G:
    ...
    def plot_density_per_road_segment(
        ...
        try:
            ...
            os.makedirs(f"{OUTPUT_DIR}/{PROJECT_NAME}/df/{output_dir}", exist_ok=True)

            density_0.to_csv(f"{OUTPUT_DIR}/{PROJECT_NAME}/df/{output_dir}/density_0_{b[0].strftime('%H:%M')}.csv")
            density_1.to_csv(f"{OUTPUT_DIR}/{PROJECT_NAME}/df/{output_dir}/density_1_{b[0].strftime('%H:%M')}.csv")
    ...
    def generate_gif(
        ...
        fp_out = f"{OUTPUT_DIR}/{PROJECT_NAME}/gifs/{class_id}_{now}_{frames_dir}.gif"
