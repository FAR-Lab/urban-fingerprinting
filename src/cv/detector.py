...
class YOLO_Detector:
    ...
    def __init__(self, TOP_LEVEL_DIR, DAY_OF_COVERAGE, YOLO_DIR='./yolov7', YOLO_WEIGHTS='./yolov7_weights/yolov7-e6e.pt', OUTPUT_DIR='../../output/yolo/'):
        ...
        self.semaphores = [asyncio.Semaphore(self.MAX_TASKS_PER_GPU) for _ in range(self.NUM_GPUS)]
    ...
    def create_batch_symlinks(self, frames_path_lists, frames_list_path):
        batch_paths = []
        for frames_list in tqdm(frames_path_lists, desc='Creating symlinks for all frames lists'): 

            batch_paths.append(f'{frames_list_path}/{os.path.basename(frames_list)}')
            # Create directory if it doesn't exist
            os.makedirs(f'{frames_list_path}/{os.path.splitext(os.path.basename(frames_list))[0]}', exist_ok=True)
    ...
    def queue_detect_tasks(self, n_workers=4, write_frames_lists=True): 
        ...
        if write_frames_lists:
            ...
            frames_dirs = glob.glob(f'{self.TOP_LEVEL_DIR}/{self.DAY_OF_COVERAGE}/*/2023*/')
            frames_paths_list = self.write_n_frames_list(frames_dirs, n_workers * self.MAX_TASKS_PER_GPU, f'{self.OUTPUT_DIR}/{self.DAY_OF_COVERAGE}/frames_lists')
            batch_paths = self.create_batch_symlinks(frames_paths_list, f'{self.OUTPUT_DIR}/{self.DAY_OF_COVERAGE}/frames_lists')
        else: 
