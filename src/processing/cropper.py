...
class Cropper:
    def __init__(self, DoC, image_dir, nexar_format=True):
        self.log = setup_logger()
        self.log.setLevel("INFO")
        self.log.info("Initializing Cropper...")
        ...
        async with s:
            ...
            try:
                annotation_path = glob(
                    f"{INSTALL_DIR}/output/yolo/{self.DoC}/*/exp/labels/{image_id}.txt"
                )[0]
            except IndexError:
            ...
            async with aiofiles.open(annotation_path, "r") as f:
                ...
                for idx, a in enumerate(annotations):
                    ...
                    # skip if aspect ratio is not ASPECT_RATIO += 0.1
                    if (w / h < (self.ASPECT_RATIO - 0.1)) or (
                        w / h > (self.ASPECT_RATIO + 0.1)
                    ):
                        self.log.info(
