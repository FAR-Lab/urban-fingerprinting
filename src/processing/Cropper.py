...
class Cropper(): 
    ...
    async def crop(self, image_path):
        ...
        # get annotation path 
        try: 
            annotation_path = glob(f"../../output/yolo/{self.DoC}/*/exp/labels/{image_id}.txt")[0]
        except IndexError:
        ...
        async with aiofiles.open(annotation_path, 'r') as f:
            ...
            for idx, a in enumerate(annotations): 
                ...
                if w * h < self.SIZE_THRESHOLD: 
                    self.log.info(f"Area of {image_id}_{idx}.png is {w*h}, less than {self.SIZE_THRESHOLD}.")
                    continue
                
                # skip if aspect ratio is not ASPECT_RATIO += 0.25
                if (w/h < (self.ASPECT_RATIO-0.25)) or (w/h > (self.ASPECT_RATIO+0.25)): 
                    self.log.info(f"Aspect ratio of {image_id}_{idx}.png is {w/h}, out of range.")
                    continue
